...
**Your current task (7) documentation:**
...
See @.taskmaster/docs/prd.txt for complete product requirements
...
git commit -m "feat(task-7): implement [brief description]
...
```bash
gh pr create --title "feat(task-7): [brief description]" \
...
- Use `gh pr list --state all --label "task-7"` to find existing PRs for your task
