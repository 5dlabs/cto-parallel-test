...
**Your current task (6) documentation:**
...
git commit -m "feat(task-6): implement [brief description]
...
```bash
gh pr create --title "feat(task-6): [brief description]" \
...
- Use `gh pr list --state all --label "task-6"` to find existing PRs for your task
