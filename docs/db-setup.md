...
- Start Postgres (example via Docker):
  `docker run -d --name cto_pg -e POSTGRES_PASSWORD=secret -e POSTGRES_DB=ecommerce_db -p 5432:5432 postgres:16-alpine`
- Create `.env` with `DATABASE_URL=postgres://postgres:secret@localhost:5432/ecommerce_db`.
