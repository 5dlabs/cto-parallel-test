...
/// `ProductService` provides thread-safe in-memory product management
...
impl ProductService {
    /// Creates a new `ProductService` with empty product list
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    /// `None` values are treated as no filter (matches everything)
    ///
    /// # Panics
    /// Panics if the mutex lock is poisoned
    #[must_use]
    pub fn filter(&self, filter: &ProductFilter) -> Vec<Product> {
        ...
        products
            ...
            .filter(|p| {
                ...
                // In stock filter: inventory_count > 0 matches in_stock=true
...
mod tests {
    ...
    fn test_product_retrieval() {
        ...
        assert_eq!(found_product.name, created.name);
    ...
    fn test_inventory_update() {
        ...
        assert_eq!(updated_product.inventory_count, 5);
    ...
    fn test_product_filtering() {
        ...
        // Test combined filters with price range
        let filtered = service.filter(&ProductFilter {
            name_contains: Some("a".to_string()),
