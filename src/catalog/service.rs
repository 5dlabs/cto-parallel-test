...
mod tests {
    ...
    fn test_filter_combined() {
        ...
        let _p1 = service.create(NewProduct {
            ...
            description: "Phone".to_string(),
            price: Decimal::from_str("799.99").unwrap(),
            inventory_count: 5,
        });
        ...
        filter.min_price = Some(Decimal::from_str("700.00").unwrap());
        filter.max_price = Some(Decimal::from_str("900.00").unwrap());
        filter.in_stock = Some(true);

        let results = service.filter(&filter);
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].name, "Apple iPhone 13");
    }
    ...
    fn test_concurrent_access() {
        ...
        let handle1 = thread::spawn(move || {
            for i in 0..10 {
                let _p = service_clone1.create(NewProduct {
                    name: format!("Product Thread1-{i}"),
        ...
        let handle2 = thread::spawn(move || {
            for i in 0..10 {
                let _p = service_clone2.create(NewProduct {
                    name: format!("Product Thread2-{i}"),
