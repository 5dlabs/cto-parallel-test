...
impl ProductService {
    ...
    ///
    /// The service initializes with an empty product list and ID counter starting at 1.
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    ///
    /// Assigns a unique ID to the product and adds it to the catalog.
    ///
    /// # Panics
    ///
    /// Panics if the mutex is poisoned (only occurs if another thread panicked while holding the lock).
    #[must_use]
    ...
    ///
    /// All filter criteria are optional (None means no filter).
