...
impl ProductService {
    ...
    ///
    /// Returns the updated product if found, or None if not found
    ///
    /// # Panics
    ///
    /// Panics if the mutex lock is poisoned
...
mod tests {
    ...
    fn test_get_all() {
        ...
        let _product1 = service.create(NewProduct {
            name: "Product 1".to_string(),
        ...
        let _product2 = service.create(NewProduct {
            name: "Product 2".to_string(),
    ...
    fn test_filter_by_price_range() {
        ...
        filter.max_price = Some(Decimal::from_str("80.00").unwrap());
    ...
    fn test_filter_combined() {
        ...
        let _gaming = service.create(NewProduct {
        ...
        let _office = service.create(NewProduct {
        ...
        let _budget = service.create(NewProduct {
    ...
    fn test_delete() {
        ...
        assert!(!service.delete(999)); // Try to delete non-existent product
    }
    ...
    fn test_concurrent_access() {
        ...
        // Spawn 10 threads that each create a product
        for i in 0..10 {
    ...
    fn test_concurrent_updates() {
        ...
        let product = service.create(NewProduct {
            ...
            description: "Test concurrent updates".to_string(),
            price: Decimal::from_str("10.00").unwrap(),
            inventory_count: 100,
        });
        ...
        // Spawn 5 threads that each update the inventory
        for i in 0..5 {
            let service_clone = Arc::clone(&service);
            let product_id = product.id;
            let handle = thread::spawn(move || {
                let _ = service_clone.update_inventory(product_id, i * 10);
            });
            handles.push(handle);
        }
        ...
        let updated = service.get_by_id(product.id);
        assert!(updated.is_some());
    }
