...
impl ProductService {
    ...
    ///
    /// All filter criteria are combined with AND logic. `None` values in the filter
    /// mean no restriction on that criterion.
    ///
    /// # Panics
    ///
    /// Panics if the internal mutex is poisoned.
...
mod tests {
    ...
    fn test_inventory_update() {
        ...
        let non_existent = service.update_inventory(9999, 100);
    ...
    fn test_product_filtering() {
        ...
        let filtered = service.filter(&ProductFilter {
            name_contains: Some("a".to_string()), // Contains "a"
            in_stock: Some(true),
            ..Default::default()
        });
